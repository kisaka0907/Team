...
func AddFriend(c *gin.Context) {
	...
	h := websocket.NewHub()
	go h.Run()
	models.RoomToHub[room.Model.ID] = h
...
func CreateGroup(c *gin.Context) {
	...
	selectedFriends := request.SelectedFriends
	fmt.Print(selectedFriends)
	room_name := user.(models.Users).UserName
	for _, friend_name := range selectedFriends {
	...
	u := models.Users{}
	result = initializers.DB.Debug().Where("id = ?", user.(models.Users).ID).First(&u)
	if result.Error != nil {
		// 一度作った部屋を消す
		initializers.DB.Where("room_name = ?", room_name).Delete(models.Rooms{})
		c.HTML(http.StatusOK, "home.html", gin.H{
	...
	initializers.DB.Debug().Where("room_name = ?", room_name).First(&room)
	if result.Error != nil {
		// 一度作った部屋を消す
		initializers.DB.Where("room_name = ?", room_name).Delete(models.Rooms{})
		c.HTML(http.StatusOK, "home.html", gin.H{
	...
	users := []models.Users{}
	result = initializers.DB.Debug().Where("user_name IN ?", selectedFriends).Find(&users)
	if result.Error != nil {
		// 一度作った部屋を消す
		initializers.DB.Where("room_name = ?", room_name).Delete(models.Rooms{})
		// 一度作ったgroup_memberを消す
		initializers.DB.Where("users_refer = ?", u.ID).Delete(models.Groups{})
		c.HTML(http.StatusOK, "home.html", gin.H{
			"title": "エラーが発生しました",
		})
		return
	}
	var u_delete []models.Users
	u_delete = append(u_delete, u)
	for _, u := range users {
		u_delete = append(u_delete, u)
		group_member = models.Groups{UsersRefer: u.ID, RoomsRefer: room.ID}
		result = initializers.DB.Debug().Create(&group_member)
		if result.Error != nil {
			// 一度作った部屋を消す
			initializers.DB.Where("room_name = ?", room_name).Delete(models.Rooms{})
			// 一度作ったgroup_memberを消す
			for j := 0; j < len(u_delete)-1; j++ {
				initializers.DB.Where("users_refer = ?", u_delete[j].ID).Delete(models.Groups{})
			}
			c.HTML(http.StatusOK, "home.html", gin.H{
				"title": "エラーが発生しました",
			})
...
func DeleteFriend(c *gin.Context) {
	user, _ := c.Get("user")
	friend_id, _ := strconv.Atoi(c.PostForm("userid"))
	friend := models.Friends{}
	conditions := "(user_id1 = ? AND user_id2 = ?) OR (user_id1 = ? AND user_id2 = ?)"
	result := initializers.DB.Debug().
		Where(conditions, friend_id, user.(models.Users).ID, user.(models.Users).ID, friend_id).Find(&friend)
	if result.Error != nil {
		c.HTML(http.StatusOK, "home.html", gin.H{
			"title": "エラーが発生しました",
		})
	...
	result = initializers.DB.Debug().
		Where(conditions, friend_id, user.(models.Users).ID, user.(models.Users).ID, friend_id).Delete(&models.Friends{})
	if result.Error != nil {
		c.HTML(http.StatusOK, "home.html", gin.H{
			"title": "エラーが発生しました",
		})
...
func DeleteGroup(c *gin.Context) {
	// user, _ := c.Get("user")
	room_name := c.PostForm("room")
	room := models.Rooms{}
	result := initializers.DB.Debug().Where("room_name=?", room_name).Find(&room)
	if result.Error != nil {
		c.HTML(http.StatusOK, "home.html", gin.H{
			"title": "エラーが発生しました",
		})
	...
	result = initializers.DB.Debug().Where("room_name = ?", room_name).Delete(&models.Rooms{})
	if result.Error != nil {
		c.HTML(http.StatusOK, "home.html", gin.H{
			"title": "エラーが発生しました",
		})
	...
	result = initializers.DB.Debug().
		Where("rooms_refer = ?", room.ID).Delete(&[]models.Groups{})
	if result.Error != nil {
		c.HTML(http.StatusOK, "home.html", gin.H{
			"title": "エラーが発生しました",
		})
